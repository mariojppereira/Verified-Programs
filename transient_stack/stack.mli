...
type 'a estack
(*@ mutable model eview: 'a memory -> 'a seq *)
...
(*@ r = ecreate [m: 'a memory] x
      ensures r.eview m = Seq.empty *)

val epush : 'a estack -> 'a -> unit
(*@ epush [m: 'a memory] s x
      modifies s
      ensures  s.eview m = Seq.cons x (old s.eview m) *)

val epop : 'a estack -> 'a
(*@ r = epop [m: 'a memory] s
      modifies s
      requires s.eview m <> Seq.empty
      ensures  (old s.eview m) = Seq.cons r (s.eview m) *)

(* Basic operations on persistent stacks. *)
type 'a pstack
(*@         model pview: 'a memory -> 'a seq
...
(*@ r, [m': 'a memory] = pcreate [m: 'a memory] x
      ensures r.pview m' = Seq.empty
      ensures extend m m' *)

val ppush : 'a pstack -> 'a -> 'a pstack
(*@ r, [m': 'a memory] = ppush [m: 'a memory] s x
      modifies s
      ensures  r.pview m' = Seq.cons x (s.pview m)
      ensures  extend m m' *)

val ppop : 'a pstack -> 'a pstack * 'a
(*@ (rs, res) = ppop [m: 'a memory] s
      modifies s
      requires s.pview m <> Seq.empty
      ensures  s.pview m = Seq.cons res (rs.pview m) *)
